...
class LegalPagesPublisher
{
    public static function publish(bool $overwriteExisting = true): void
    {
        ...
        foreach (self::definitions() as $page) {
            ...
            $existingData = self::decodeData($existing?->data);
            $data = $overwriteExisting
                ? array_replace($existingData, $page['data'])
                : self::mergeMissingData($existingData, $page['data']);

            $payload = [
                'title' => $overwriteExisting
                    ? $page['title']
                    : self::preferExistingScalar($existing?->title, $page['title']),
    ...
    public static function publishMissing(): void
    {
        self::publish(false);
    }
