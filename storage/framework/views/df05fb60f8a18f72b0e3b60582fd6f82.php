    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-100">
        ...
        <div
            ...
            <div class="p-4 border-b border-gray-100">
                <h2 class="font-bold text-gray-800 text-lg leading-tight"><?php echo e($course->title); ?></h2>
                <div class="mt-2 w-full bg-gray-200 rounded-full h-2">
                    <div class="bg-green-500 h-2 rounded-full transition-all duration-500"
                        style="width: <?php echo e($percentage); ?>%"></div>
                </div>
                <p class="text-xs text-gray-500 mt-1"><?php echo e($percentage); ?>% Concluído</p>
            </div>
            <div class="py-2">
                <?php $__currentLoopData = $course->lessons()->orderBy('order')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route('courses.lessons.show', [$course->id, $l->id])); ?>"
                        class="flex items-center p-4 hover:bg-gray-50 transition border-l-4 <?php echo e($l->id == $lesson->id ? 'border-[#1F5EDB] bg-blue-50' : 'border-transparent'); ?>">
                        <div class="mr-3">
                            <?php if(isset($lessonProgressMap[$l->id]) && $lessonProgressMap[$l->id]): ?>
                        ...
                        <div>
                            <p class="text-sm font-medium <?php echo e($l->id == $lesson->id ? 'text-[#1F5EDB]' : 'text-gray-700'); ?>">
                                <?php echo e($l->order); ?>. <?php echo e($l->title); ?>

                            </p>
                            <?php if($l->duration > 0): ?>
                                <p class="text-xs text-gray-400 mt-1">
                                    <i class="far fa-clock mr-1"></i> <?php echo e(gmdate("H:i", $l->duration)); ?>
        ...
        <div class="flex-1 p-6 md:p-10 overflow-y-auto">
            <div class="max-w-4xl mx-auto">
                <h1 class="text-2xl font-bold text-gray-900 mb-6"><?php echo e($lesson->title); ?></h1>
                ...
                <div class="relative w-full overflow-hidden shadow-2xl mb-8 bg-black rounded-xl" style="aspect-ratio: 16/9;"
                    ...
                    <?php if($lesson->video_url): ?>
                        ...
                        ?>
                        <?php $usePlyr = (string) \App\Models\Setting::get('video_player_enabled', '1') === '1'; ?>
                        <?php if($usePlyr): ?>
                            <div class="absolute inset-0" data-unn-video-player data-video-url="<?php echo e($normalizedVideoUrl); ?>"
                                data-progress-url="<?php echo e(route('courses.lessons.progress.update', [$course->id, $lesson->id])); ?>"
                                data-bookmark-store-url="<?php echo e(route('courses.lessons.bookmarks.store', [$course->id, $lesson->id])); ?>"
                                data-bookmark-delete-url-template="<?php echo e(route('courses.lessons.bookmarks.destroy', [$course->id, $lesson->id, '__BOOKMARK_ID__'])); ?>"
                                data-resume-at="<?php echo e((int) ($resumeAt ?? 0)); ?>"
                                data-block-download="<?php echo e(!empty($course->video_block_download) ? '1' : '0'); ?>"
                                data-floating-enabled="<?php echo e(!empty($course->video_floating_enabled) ? '1' : '0'); ?>"
                                data-floating-width="<?php echo e((int) ($course->video_floating_width ?? 420)); ?>"
                                data-floating-height="<?php echo e((int) ($course->video_floating_height ?? 236)); ?>">
                                <video class="w-full h-full object-contain" controls playsinline preload="metadata">
                                    <source src="<?php echo e($normalizedVideoUrl); ?>">
                                </video>
                            </div>
                        <?php else: ?>
                            <iframe src="<?php echo e(str_replace('youtu.be/', 'youtube.com/embed/', $normalizedVideoUrl)); ?>" frameborder="0"
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-8">
                    ...
                    <div class="prose max-w-none text-gray-700">
                        <?php echo \App\Support\RichText::toHtml($lesson->content); ?>
                            ...
                            <div id="lessonBookmarkList" class="space-y-2">
                                <?php $__empty_1 = true; $__currentLoopData = $bookmarks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $bookmark): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <div class="flex items-start justify-between gap-3 p-3 border border-gray-200 rounded-lg"
                                        data-bookmark-id="<?php echo e($bookmark->id); ?>"
                                        data-bookmark-seconds="<?php echo e((int) $bookmark->position_seconds); ?>">
                                        <div class="min-w-0">
                                            <button type="button"
                                                class="text-sm font-bold text-[#1F5EDB] hover:underline text-left lesson-bookmark-jump">
                                                <i
                                                    class="fas fa-play-circle mr-1"></i><?php echo e($formatSeconds($bookmark->position_seconds)); ?>

                                            </button>
                                            <p class="text-sm text-gray-700 mt-1 break-words"><?php echo e($bookmark->note); ?></p>
                                        </div>
                                        <button type="button"
                        ...
                        <div class="mt-8 pt-6 border-t border-gray-100">
                            ...
                            <div class="grid gap-3">
                                <?php $__currentLoopData = $lesson->attachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $attachment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    ?>
                                    <a href="<?php echo e(route('courses.lessons.attachments.download', [$course->id, $lesson->id, $attachment->id])); ?>"
                                        download="<?php echo e($attachment->file_name); ?>"
                                        ...
                                        <div class="flex items-center overflow-hidden">
                                            <div
                                                ...
                                                <i class="fas <?php echo e($attachmentIcon); ?>"></i>
                                            </div>
                                            <div class="truncate">
                                                <p
                                                ...
                                                <p class="text-xs text-gray-500">
                                                    <?php echo e(round($attachment->file_size / 1024 / 1024, 2)); ?> MB • <?php echo e($attachmentBadge); ?>
                ...
                <div class="flex justify-between items-center">
                    <?php if($previous): ?>
                        <a href="<?php echo e(route('courses.lessons.show', [$course->id, $previous->id])); ?>"
                    ...
                    <?php if($next): ?>
                        <a href="<?php echo e(route('courses.lessons.show', [$course->id, $next->id])); ?>"
                    ...
                    <?php else: ?>
                        ...
                        <?php if($userCertificate): ?>
                            <a href="<?php echo e(route('admin.certificates.view', $userCertificate->cert_hash)); ?>" target="_blank"
                        ...
                        <?php else: ?>
                            <?php if($percentage >= 89): ?>
                                <form action="<?php echo e(route('courses.complete', $course->id)); ?>" method="POST" class="d-inline"
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH G:\Tudo\MEU-SISTEMA\SOMOS_UNN\resources\views\courses\lesson.blade.php ENDPATH**/ ?>