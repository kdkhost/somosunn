                        ...
                        <div class="border-t border-gray-100 mt-6 pt-6">
                            @if($isPaid)
                                ...
                                @if($flashActive && $event->flash_sale_ends_at)
                                    <div
                                        class="mt-2 inline-flex items-center gap-2 rounded-full bg-rose-50 border border-rose-200 px-3 py-1 text-xs font-black text-rose-800">
                                ...
                                @if(($mpEnabled ?? false) || ($psEnabled ?? false))
                                    <div
                                        class="mt-4 pt-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                                        <span class="text-sm text-gray-500">Você está pagando com:</span>
                                        <span id="display_selected_gateway"
                                            class="text-sm font-bold text-gray-900 bg-gray-100 px-3 py-1 rounded-full text-center">
                                            {{ (($preferredGateway ?? null) !== 'pagseguro') ? 'Mercado Pago' : 'PagSeguro' }}
                                        </span>
                                    </div>
                                @endif
                            @else
                ...
                <div class="lg:col-span-2">
                    ...
                    @if($alreadyConfirmed)
                        <div class="bg-white rounded-2xl shadow-lg p-8">
                            ...
                            <a href="{{ route('events.show', $event) }}"
                                class="inline-flex items-center gap-2 btn-primary px-6 py-3 rounded-xl font-bold">
                            ...
                            <form action="{{ route('events.reserve', $event) }}" method="POST" class="space-y-6">
                                ...
                                @if($isPaid)
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-2">Se tiver um cupom, aplique aqui antes do pagamento.
                                        </p>
                                    </div>
                                ...
                                @guest
                                    <div class="border-t border-gray-100 pt-6">
                                        ...
                                        <div class="grid md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-2">CPF
                                                    {{ $isPaid ? '(obrigatório)' : '(opcional)' }}</label>
                                            ...
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Telefone
                                                    (opcional)</label>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                if (!display || radios.length === 0) return;
                ...
                function updateGatewayDisplay() {
                    ...
                    if (selected) {
                        ...
                        } else if (selected.value === 'pagseguro') {
                            display.innerText = 'PagSeguro';
                        }
                    }
                }
                ...
                // Set on load just in case logic changed default checks
