        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="hidden md:block">
                <div class="bg-white rounded-lg shadow p-4 sticky top-24">
                    ...
                    <nav class="space-y-2">
                        <a href="{{ route('social.feed') }}" class="flex items-center gap-2 text-blue-600 font-medium p-2 bg-blue-50 rounded">
                            <i class="fas fa-newspaper w-6"></i> Feed
                        </a>
                        <a href="{{ route('chat.index') }}" class="flex items-center gap-2 text-gray-600 hover:text-blue-600 p-2 rounded transition">
                            <i class="fas fa-comments w-6"></i> Mensagens
                        </a>
                        <a href="{{ route('social.profile', Auth::id()) }}" class="flex items-center gap-2 text-gray-600 hover:text-blue-600 p-2 rounded transition">
                            <i class="fas fa-user w-6"></i> Meu Perfil
                        </a>
                        <a href="{{ route('courses.index') }}" class="flex items-center gap-2 text-gray-600 hover:text-blue-600 p-2 rounded transition">
                ...
                <div class="bg-white rounded-lg shadow p-4">
                    <form action="{{ route('social.post.store') }}" method="POST">
                        ...
                        <div class="flex gap-3">
                            <div class="bg-blue-600 text-white rounded-full w-10 h-10 flex-shrink-0 flex items-center justify-center font-bold">
                                {{ substr(Auth::user()->name, 0, 1) }}
                            </div>
                            <div class="flex-1">
                                <textarea name="content" rows="3" class="w-full border-gray-100 rounded-lg focus:ring-blue-500 focus:border-blue-500 bg-gray-50 p-3" placeholder="No que você está pensando?"></textarea>
                            </div>
                        </div>
                        <div class="flex justify-between items-center mt-3 pt-3 border-t">
                            <div class="flex gap-2">
                                <button type="button" class="text-gray-500 hover:text-blue-600 p-2 rounded hover:bg-gray-100">
                            ...
                            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-full font-medium hover:bg-blue-700 transition">
                                Publicar
                            </button>
                        </div>
                    </form>
