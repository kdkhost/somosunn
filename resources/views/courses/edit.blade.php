...
<div class="max-w-3xl mx-auto px-4 py-10">
    ...
    <form action="{{ route('courses.update', $course->id) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded-lg p-8 space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ $course->title }}" required class="w-full rounded-md border-gray-300 shadow-sm">
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Preço (R$)</label>
                <input type="number" step="0.01" name="price" value="{{ $course->price }}" required class="w-full rounded-md border-gray-300 shadow-sm">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Duração (min)</label>
                <input type="number" name="duration" value="{{ $course->duration }}" class="w-full rounded-md border-gray-300 shadow-sm">
            </div>
        </div>

         <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Descrição Curta</label>
            <textarea name="short_description" rows="2" class="w-full rounded-md border-gray-300 shadow-sm">{{ $course->short_description }}</textarea>
        </div>
        ...
        <div class="pt-4 border-t flex justify-end">
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md shadow-sm hover:bg-blue-700 transition">
                Salvar Alterações
            </button>
        </div>
    </form>
            ...
            <form action="{{ route('courses.lessons.store', $course->id) }}" method="POST" class="mb-6 border-b pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    <div class="md:col-span-1">
                        ...
                        <input type="number" name="order" value="{{ $course->lessons->count() + 1 }}" class="w-full rounded text-sm border-gray-300">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <button type="submit" class="w-full py-2 bg-green-600 text-white text-sm font-bold rounded hover:bg-green-700">
                            + Adicionar
                        </button>
                    </div>
                </div>
                ...
                @foreach($course->lessons()->orderBy('order')->get() as $lesson)
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded border border-gray-100">
                        ...
                        <div class="flex items-center gap-2">
                            <form action="{{ route('courses.lessons.destroy', [$course->id, $lesson->id]) }}" method="POST" onsubmit="return confirm('Excluir?');">
                                @csrf @method('DELETE')
