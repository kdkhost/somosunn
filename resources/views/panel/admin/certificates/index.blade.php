...
@section('panel_content')
    <div x-data="{ tab: 'issued' }" class="space-y-6">
        ...
        {{-- Tabs Navigation --}}
        <div
            class="bg-white dark:bg-slate-900 p-1 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 inline-flex items-center gap-1 transition-colors">
            <button @click="tab = 'issued'"
                :class="tab === 'issued' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-200'"
                class="px-4 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2">
                <i class="fas fa-certificate"></i>
                <span>Emitidos</span>
            </button>
            <button @click="tab = 'pending'"
                :class="tab === 'pending' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-200'"
                class="px-4 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2">
