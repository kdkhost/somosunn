...
<div x-data="{ tab: 'general' }" class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-4">
            <a href="{{ route('panel.admin.courses.index') }}" 
               class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-500 hover:text-slate-900 hover:border-slate-300 transition-all shadow-sm">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('courses.show', $course->slug ?: $course->id) }}" target="_blank"
               class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition-all shadow-sm">
    ...
    <div class="bg-white p-1 rounded-2xl shadow-sm border border-slate-200 inline-flex items-center gap-1">
        ...
        <button @click="tab = 'lessons'" 
                :class="tab === 'lessons' ? 'bg-blue-50 text-blue-600' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'"
                class="px-4 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2">
    ...
    <form id="courseForm" method="POST" 
        ...
        <div x-show="tab === 'general'" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Título do Curso</label>
                            <input type="text" name="title" value="{{ old('title', $course->title) }}" required
                                   class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-lg font-bold"
                                   placeholder="Ex: Domínio do Backend com Laravel">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Descrição Curta</label>
                            <textarea name="short_description" rows="3" maxlength="500"
                                      class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all resize-none"
                                      placeholder="Um breve resumo que aparece nos cards de curso...">{{ old('short_description', $course->short_description) }}</textarea>
                            <p class="text-[10px] text-slate-400 mt-1">Máximo 500 caracteres.</p>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Descrição Completa</label>
                            <textarea name="full_description" id="fullDescription" rows="10"
                                      class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">{{ old('full_description', $course->full_description) }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-start gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
                            ...
                            <div>
                                <label for="video_block_download" class="block text-sm font-bold text-slate-900">Bloquear download do vídeo</label>
                                <p class="text-xs text-slate-500">Remove botões/menus de download nativos do player.</p>
                            </div>
                        </div>

                        <div class="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-4">
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    <label for="video_floating_enabled" class="block text-sm font-bold text-slate-900">Mini player flutuante</label>
                                    <p class="text-xs text-slate-500">Ao rolar a página, o vídeo fica fixo no canto da tela.</p>
                                </div>
                            </div>

                            <div x-show="floating" x-transition class="grid grid-cols-2 gap-4 ml-8">
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-400 uppercase">Largura (px)</label>
                                    <input type="number" name="video_floating_width" value="{{ $course->video_floating_width ?? 420 }}"
                                           class="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 outline-none">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-bold text-slate-400 uppercase">Altura (px)</label>
            ...
            <div class="space-y-6">
                {{-- Publish Action --}}
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <button type="submit" class="w-full px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-200 transition-all flex items-center justify-center gap-2 mb-4">
                    ...
                    <div class="space-y-4 pt-4 border-t border-slate-100">
                        <div>
                            ...
                            <select name="status" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm font-medium">
                        ...
                        <div class="flex items-center gap-3">
                            <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ $course->is_featured ? 'checked' : '' }}
                                   class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
                            <label for="is_featured" class="text-sm font-semibold text-slate-700">Destaque na Home</label>
                        </div>
                        
                        <div class="flex items-center gap-3">
                            <input type="checkbox" name="is_certificate_enabled" id="is_certificate_enabled" value="1" {{ $course->is_certificate_enabled ? 'checked' : '' }}
                                   class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
                            <label for="is_certificate_enabled" class="text-sm font-semibold text-slate-700">Habilitar Certificado</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    ...
                    <div x-data="{ photoName: null, photoPreview: '{{ $course->thumbnail ? asset($course->thumbnail) : '' }}' }" class="space-y-4">
                        <div class="relative group">
                            <div class="aspect-video w-full rounded-xl bg-slate-100 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden transition-all group-hover:border-blue-300">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Instrutor / Autor</label>
                    <input type="text" name="author_name" value="{{ old('author_name', $course->author_name ?? Auth::user()->name) }}"
                           class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium">
                </div>
            </div>
        </div>
        ...
        <div x-show="tab === 'pricing'" class="max-w-3xl space-y-6">
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1 text-center">Preço Padrão</label>
                        <div class="relative group">
                            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold group-focus-within:text-blue-500">R$</span>
                            <input type="text" name="price" value="{{ old('price', $course->price) }}"
                                   class="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-xl font-bold mask-money"
                                   placeholder="0,00">
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden group">
                <div class="absolute top-0 right-0 p-4">
                    <i class="fas fa-bolt text-4xl text-amber-100 group-hover:text-amber-200 transition-colors"></i>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Preço Promocional</label>
                        <div class="relative peer group">
                            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold group-focus-within:text-amber-500 transition-colors">R$</span>
                            <input type="text" name="flash_sale_price" value="{{ old('flash_sale_price', $course->flash_sale_price) }}"
                                   class="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all text-xl font-bold mask-money"
                                   placeholder="0,00">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Data de Expiração</label>
                        <input type="datetime-local" name="flash_sale_ends_at" 
                               ...
                               class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all text-sm font-medium">
                        <p class="text-[10px] text-slate-400 mt-2 italic">A promoção será encerrada automaticamente nesta data e o preço voltará ao valor padrão.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="tab === 'lessons'" class="space-y-6">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-bold text-slate-900">Grade Curricular</h3>
            ...
            <div id="lessons-list" class="space-y-3">
                @forelse($course->lessons as $lesson)
                    <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex items-center gap-4 group hover:border-blue-300 transition-all" data-id="{{ $lesson->id }}">
                        <div class="cursor-move text-slate-400 hover:text-slate-600 transition-colors">
                            <i class="fas fa-grip-vertical"></i>
                        </div>
                        <div class="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center text-xs font-bold text-slate-400 border border-slate-200">
                            #{{ $lesson->order }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <h4 class="font-bold text-slate-900 truncate">{{ $lesson->title }}</h4>
                            <div class="flex items-center gap-3 mt-0.5">
                                ...
                                <span class="text-[10px] font-bold uppercase tracking-wider text-blue-500 flex items-center gap-1">
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button type="button" @click="editLesson({{ $lesson->id }})"
                                    class="p-2 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors border border-transparent hover:border-blue-100 text-slate-400"
                                    title="Editar Aula">
                            ...
                            <form action="{{ route('courses.lessons.destroy', [$course, $lesson]) }}" method="POST"
                                ...
                                @csrf @method('DELETE')
                                <button type="submit"
                                        class="p-2 hover:bg-red-50 hover:text-red-700 rounded-lg transition-colors border border-transparent hover:border-red-100 text-slate-400"
                                        title="Excluir Aula">
                ...
                @empty
                    <div class="text-center py-12 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                        <i class="fas fa-layer-group text-4xl text-slate-200 mb-3"></i>
                        <p class="text-slate-500 text-sm">Nenhuma aula cadastrada. Comece adicionando o conteúdo!</p>
                    </div>
                @forelse
            </div>
        </div>
        ...
        <div x-show="tab === 'certificate'" class="space-y-6">
            ...
            <div class="alert alert-info bg-blue-50 border-blue-100 text-blue-700 rounded-2xl p-4 flex items-start gap-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-3">
                    <div class="bg-slate-900 rounded-2xl p-8 border border-slate-800 shadow-2xl relative overflow-hidden flex items-center justify-center" style="min-height: 500px;">
                        {{-- Editor Canvas --}}
                        <div id="cert-canvas" 
                ...
                <div class="space-y-6">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        <h4 class="text-sm font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">Plano de Fundo</h4>
                        
                        <div class="space-y-4">
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Imagem de Fundo</label>
                            ...
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Ajuste</label>
                                <select id="cert-bg-fit" class="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-blue-500/20 outline-none">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                        <h4 class="text-sm font-bold text-slate-900 mb-4 border-b border-slate-100 pb-2">Assinatura</h4>
                        
                        <div class="space-y-4">
                            <div class="aspect-[3/1] rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden">
                            ...
                            <input type="file" name="instructor_signature" accept="image/png"
                                   class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all cursor-pointer">
                            <p class="text-[9px] text-slate-400 italic">Recomendado: PNG com fundo transparente.</p>
                        </div>
                    </div>
                </div>
            </div>
...
<div id="lesson-modal" class="fixed inset-0 z-[100] hidden items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
    <div class="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
            <h3 class="text-xl font-bold text-slate-900">Gerenciar Aula</h3>
            <button @click="closeModal('lesson-modal')" class="text-slate-400 hover:text-slate-600 transition-colors">
        ...
        <div class="p-6 overflow-y-auto flex-1 h-full space-y-6">
            <form id="lessonForm" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Ordem</label>
                        <input type="number" name="order" id="lessonOrder" value="1"
                               class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none text-sm">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Duração (s)</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Link do Vídeo (YT/Vimeo)</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Conteúdo</label>
                    <textarea name="content" id="lessonContent" rows="4"
                              class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 outline-none text-sm"></textarea>
                </div>

                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_free_preview" id="lessonPreview" value="1"
                           class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
                    <label for="lessonPreview" class="text-sm font-semibold text-slate-700">Aula Gratuita (Preview)</label>
                </div>

                <div class="pt-4 border-t border-slate-100">
                    <h4 class="text-sm font-bold text-slate-900 mb-3">Materiais de Apoio</h4>
                    <input type="file" name="file" class="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all cursor-pointer">
        ...
        <div class="p-6 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3">
            <button @click="closeModal('lesson-modal')" class="px-6 py-2 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-200 transition-colors">
                Cancelar
            </button>
            <button type="button" id="btnSaveLesson" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-200 transition-all">
                Salvar Aula
            </button>
        </div>
    </div>
