    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p class="text-slate-500 text-sm">Controle de acesso, planos e permissões dos membros.</p>
            </div>
        ...
        <div class="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
            <a href="{{ route('panel.admin.users.index') }}" 
               class="px-4 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap transition {{ !request('role') ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200' }}">
               Todos
            </a>
            @php
                $roles = [
            ...
            @foreach($roles as $key => $label)
                <a href="{{ route('panel.admin.users.index', array_merge(request()->all(), ['role' => $key])) }}" 
                   class="px-4 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap transition {{ request('role') == $key ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200' }}">
                   {{ $label }}
                </a>
            @endforeach
        </div>
                            ...
                            <tr class="hover:bg-slate-50 transition group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 font-bold text-lg overflow-hidden shrink-0">
                                            @if($u->photo)
                                    ...
                                    <div class="flex flex-col gap-1 items-start">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-bold 
                                            {{ $u->role == 'superadmin' ? 'bg-purple-100 text-purple-700' : 
                                               ...
                                               ($u->role == 'instrutor' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700')) }}">
                                            {{ ucfirst($u->role ?? 'Membro') }}
                                        </span>
                                        @if($u->level)
                                            <span class="text-xs text-slate-400 capitalize">Nível: {{ $u->level }}</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($u->plan)
                                        <span class="font-medium text-slate-700">{{ $u->plan->name }}</span>
                                        @if($u->plan_expires_at)
                                            <div class="text-xs text-slate-400 mt-0.5">Expira em: {{ $u->plan_expires_at->format('d/m/Y') }}</div>
                                        @endif
                                    @else
                                        <span class="text-slate-400 italic">Sem plano</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @if($u->plan_id)
                                        @if($user->plan_expires_at && $user->plan_expires_at->isPast())
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700">Expirado</span>
                                        @else
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700">Ativo</span>
                                        @endif
                                    @else
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-500">-</span>
                                    @endif
                                </td>
                                    ...
                                    <div class="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                        @if(auth()->user()->role === 'superadmin' && $u->id !== auth()->id())
                                            <a href="{{ route('admin.users.impersonate', $u) }}" 
                                               class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-purple-600 hover:bg-purple-50 transition" 
