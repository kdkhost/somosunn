            ...
            @forelse($galleryMedia as $item)
                <div class="relative group aspect-square rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-900 shadow-sm border border-gray-100 dark:border-gray-700 gallery-item" data-id="{{ $item->id }}">
                    ...
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button type="button" 
                                class="p-2 bg-red-600 hover:bg-red-700 text-white rounded-full shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 btn-delete-media ml-1"
                        ...
                        @if($item->type === 'image')
                        <button type="button" 
                                class="p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 btn-set-cover mr-1"
                                data-id="{{ $item->id }}"
                                data-url="{{ route('panel.admin.events.set-cover', $model) }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    document.querySelectorAll('.btn-set-cover').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            Swal.fire({
                ...
                text: "Esta imagem será a principal do álbum.",
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    axios.post(url, { media_id: mediaId }, { headers: token ? { 'X-CSRF-TOKEN': token } : {} })
                        .then((response) => {
                            Swal.fire({
                                icon: 'success',
                                title: 'Sucesso!',
                        ...
                        .catch((error) => {
                            Swal.fire({
                                icon: 'error',
                                title: 'Erro!',
                                text: error.response?.data?.message || 'Não foi possível definir a capa.'
                            });
                        });
                }
            });
