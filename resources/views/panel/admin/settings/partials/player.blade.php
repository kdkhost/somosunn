...
<div class="space-y-6">
    <div class="flex items-center justify-between p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/30">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h4 class="font-bold text-slate-800 dark:text-white">Player de Video Plyr</h4>
                <p class="text-sm text-slate-500 dark:text-slate-400">Ative ou desative o player personalizado para as aulas.</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 space-y-4">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h3 class="font-bold text-slate-800 dark:text-white">Aparencia</h3>
            </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 space-y-4">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h3 class="font-bold text-slate-800 dark:text-white">Controles Visiveis</h3>
            </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 p-8 space-y-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <div>
                <h3 class="font-bold text-slate-800 dark:text-white">Protecao de Conteudo</h3>
                <p class="text-xs text-slate-500 dark:text-slate-400">Configure a watermark das imagens e a protecao dos videos.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div class="space-y-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                <div class="flex justify-between items-center">
                    <label class="text-sm font-bold text-slate-700 dark:text-slate-300">Marca d'agua das imagens</label>
                    <label for="image_watermark_enabled" class="relative inline-flex items-center cursor-pointer scale-75">
                        <input type="hidden" name="image_watermark_enabled" value="0">
                        <input type="checkbox" name="image_watermark_enabled" id="image_watermark_enabled" value="1" class="sr-only peer" {{ ($settings['image_watermark_enabled'] ?? 1) ? 'checked' : '' }}>
                        <div class="w-11 h-6 bg-slate-200 dark:bg-slate-800 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                    </label>
                </div>

                <div class="rounded-2xl border border-blue-100 dark:border-blue-900/40 bg-blue-50/60 dark:bg-blue-900/10 p-4 text-sm text-slate-600 dark:text-slate-300">
                    Envie uma marca d'agua em <strong>PNG</strong> ou <strong>WEBP</strong> com fundo transparente.
                    O sistema vai usar esse arquivo nas fotos publicadas, sem o bloco escuro do logo institucional.
                </div>

                <div class="flex justify-between items-center">
                    <label class="text-sm font-bold text-slate-700 dark:text-slate-300">Usar a mesma imagem no player de video</label>
                ...
                <div class="flex flex-col items-center gap-4">
                    <div class="w-full h-40 rounded-2xl watermark-preview-surface border-2 border-dashed border-slate-200 dark:border-slate-800 flex items-center justify-center overflow-hidden relative group">
                        @if($url = $getUrl('watermark_image'))
                            <img id="watermark_preview" src="{{ $url }}" class="max-h-[120px] max-w-[180px] object-contain">
                            <div id="watermark_placeholder" class="hidden text-center p-4">
                                <i class="fas fa-image text-2xl text-slate-300 mb-1"></i>
                                <p class="text-[10px] text-slate-400">Sem imagem</p>
                            </div>
                        @else
                            <img id="watermark_preview" src="" class="hidden max-h-[120px] max-w-[180px] object-contain">
                            <div id="watermark_placeholder" class="text-center p-4">
                                <i class="fas fa-image text-2xl text-slate-300 mb-1"></i>
                                <p class="text-[10px] text-slate-400">Sem imagem transparente</p>
                            </div>
                        @endif
                    </div>
                    <div class="flex gap-2 w-full">
                        ...
                        <input type="file" id="input_watermark" name="watermark_image" class="hidden" accept=".png,.webp" onchange="previewImage(this, 'watermark_preview', 'watermark_placeholder')">
                        <input type="hidden" name="remove_watermark_image" id="remove_watermark_image" value="0">
                        <button type="button" onclick="clearWatermarkPreview()"
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase mb-1">Posicao</label>
                        <select name="image_watermark_position" class="w-full px-4 py-2.5 rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 focus:border-blue-600 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all text-sm font-medium text-slate-800 dark:text-white">
                    ...
                    <div>
                        ...
                        <input type="number" min="5" max="100" name="image_watermark_opacity" value="{{ $imageWatermarkOpacity }}"
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase mb-1">Tamanho (%)</label>
                        <input type="number" min="1" max="60" name="image_watermark_size_percent" value="{{ $imageWatermarkSize }}"
                            class="w-full px-4 py-2.5 rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 focus:border-blue-600 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all text-sm font-medium text-slate-800 dark:text-white">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 uppercase mb-1">Margem (px)</label>
                        <input type="number" min="0" max="300" name="image_watermark_margin" value="{{ $imageWatermarkMargin }}"
                            class="w-full px-4 py-2.5 rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 focus:border-blue-600 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all text-sm font-medium text-slate-800 dark:text-white">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                <div class="flex justify-between items-center">
                    <label class="text-sm font-bold text-slate-700 dark:text-slate-300">Marca d'agua dinamica</label>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 p-6">
        <button type="button" onclick="document.getElementById('advancedJson').classList.toggle('hidden')"
            ...
            <i class="fas fa-cogs"></i> Configuracoes Avancadas (JSON)
        </button>
        <div id="advancedJson" class="hidden mt-4">
            ...
            <p class="text-xs text-slate-400 mt-2">Cuidado: o JSON gerado pelos checkboxes acima sobrescrevera este campo ao salvar, a menos que voce modifique o script JS.</p>
        </div>
    </div>
</div>
...
<script>
    ...
    function previewImage(input, previewId, placeholderId) {
        ...
        const removeInput = document.getElementById('remove_watermark_image');
    ...
    function clearWatermarkPreview() {
        ...
        const placeholder = document.getElementById('watermark_placeholder');
        const removeInput = document.getElementById('remove_watermark_image');
        const fileInput = document.getElementById('input_watermark');
