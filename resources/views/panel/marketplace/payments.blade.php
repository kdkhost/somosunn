...
@section('panel_content')
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 mt-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                <div class="font-extrabold text-slate-900">{{ $paymentsConfigured ? 'MercadoPago habilitado' : 'MercadoPago não configurado' }}</div>
    ...
    @if($webhookUrl !== '')
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 mt-6">
            ...
            <div class="mt-4 flex flex-col md:flex-row gap-3">
                <input id="webhook-url" type="text" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700 bg-white" readonly value="{{ $webhookUrl }}">
