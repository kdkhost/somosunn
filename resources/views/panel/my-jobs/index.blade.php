    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1 transition-colors">Gerencie as oportunidades
                    publicadas pela sua empresa.</p>
            </div>

            <div class="flex items-center gap-3">
                <a href="{{ route('panel.my-jobs.create') }}"
                        ...
                        @forelse($vacancies as $vacancy)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-950/30 transition-all group">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span
                                        class="text-xs font-bold uppercase tracking-wider text-slate-500 px-2 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                                        {{ $vacancy->visibility == 'internal' ? 'Comunidade' : ($vacancy->visibility == 'external' ? 'Público' : 'Ambos') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div
                                        ...
                                        <form action="{{ route('panel.my-jobs.destroy', $vacancy) }}" method="POST"
        ...
        <script>
            document.querySelectorAll('.btn-delete').forEach(btn => {
                btn.addEventListener('click', function () {
                    Swal.fire({
                        title: 'Tem certeza?',
                        text: "Esta vaga será removida permanentemente!",
                        icon: 'warning',
