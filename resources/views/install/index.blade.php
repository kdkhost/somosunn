    ...
    <div class="max-w-6xl mx-auto px-6 py-8 md:py-10 relative z-10 space-y-8">
        ...
        <div class="grid lg:grid-cols-2 gap-8 items-stretch min-h-[640px]">
                            ...
                            <div class="step-card p-4 rounded-2xl border border-transparent bg-gradient-to-br from-white to-slate-50 hover:from-purple-50 hover:border-purple-100 transition-all">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">{{ $step['detail'] }}</p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <ul class="space-y-3">
                        @foreach($requirements as $requirement)
                            <li class="flex items-center justify-between rounded-2xl px-4 py-3 border border-slate-100 bg-slate-50">
                                <div>
                                    <p class="text-sm font-semibold text-gray-800">{{ $requirement['label'] }}</p>
                                    <p class="text-xs text-gray-500">{{ $requirement['detail'] }}</p>
                                </div>
                                <span class="px-3 py-1 rounded-full text-xs font-semibold {{ $requirement['ok'] ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-600' }}">
                                    {{ $requirement['ok'] ? 'OK' : 'Corrija' }}
                                </span>
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="grid gap-6 bg-white/90 border border-white shadow-2xl rounded-3xl p-8 relative overflow-hidden lg:grid-cols-1 h-full">
                ...
                <form method="POST" action="{{ $installRunRoute }}" class="space-y-6 w-full h-full flex flex-col">
                    @csrf
                    <div class="space-y-6">
                        <div class="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-xl backdrop-blur flex flex-col gap-4">
                            ...
                            <div class="grid md:grid-cols-2 gap-4">
                                <div class="md:col-span-2">
                                    ...
                                    <input name="app_url" value="{{ $appUrlField }}" required class="mt-1 w-full border border-gray-200 rounded-2xl px-4 py-3 focus:border-[#55A0FF] focus:ring-2 focus:ring-[#3B5AF4]/40" placeholder="https://somosunn.com.br">
                                </div>
                                <div>
                                    <label class="text-xs font-semibold text-gray-600">Host</label>
                                    <input name="db_host" value="{{ old('db_host', env('DB_HOST')) }}" required class="w-full border border-gray-200 rounded-2xl px-4 py-3 focus:border-[#55A0FF] focus:ring-2 focus:ring-[#3B5AF4]/40" placeholder="127.0.0.1">
                                </div>
                                <div>
                                    <label class="text-xs font-semibold text-gray-600">Porta</label>
                                    <input name="db_port" value="{{ old('db_port', env('DB_PORT', '3306')) }}" required class="w-full border border-gray-200 rounded-2xl px-4 py-3 focus:border-[#55A0FF] focus:ring-2 focus:ring-[#3B5AF4]/40" placeholder="3306">
                                </div>
                                <div>
                                    <label class="text-xs font-semibold text-gray-600">Banco</label>
                                    <input name="db_database" value="{{ old('db_database', env('DB_DATABASE')) }}" required class="w-full border border-gray-200 rounded-2xl px-4 py-3 focus:border-[#55A0FF] focus:ring-2 focus:ring-[#3B5AF4]/40" placeholder="unn_db">
                                </div>
                                <div>
                                    <label class="text-xs font-semibold text-gray-600">Usuário</label>
                                    <input name="db_username" value="{{ old('db_username', env('DB_USERNAME')) }}" required class="w-full border border-gray-200 rounded-2xl px-4 py-3 focus:border-[#55A0FF] focus:ring-2 focus:ring-[#3B5AF4]/40" placeholder="root">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="text-xs font-semibold text-gray-600">Senha</label>
                                    <input name="db_password" type="password" value="{{ old('db_password', env('DB_PASSWORD')) }}" class="w-full border border-gray-200 rounded-2xl px-4 py-3 focus:border-[#55A0FF] focus:ring-2 focus:ring-[#3B5AF4]/40" placeholder="••••••••">
                                </div>
                                <div class="md:col-span-2">
