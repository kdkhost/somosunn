...
- **Integração Gateway SumUp (10/03):** Suporte completo ao gateway SumUp no Marketplace, permitindo que membros configurem seus Access Tokens diretamente pelo painel (/painel/marketplace/pagamentos/configurar) com validação de credenciais em tempo real.
- **Atalhos de Upload Rápido e Galeria (10/03):** Implementado botão "Registrar Fotos" no Sidebar e Dashboard administrativo, ícone de câmera no Navbar global e botão no Dashboard do membro. O Modal de Upload é inteligente: busca eventos dinamicamente e detecta o contexto (Admin/Painel) para aplicar as permissões e rotas corretas. A galeria de eventos agora possui uploader premium com barra de progresso e drag-and-drop.
