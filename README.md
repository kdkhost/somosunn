...
- **Correção de Sintaxe na Página de Indicação (22/03):** Resolvido o erro "unexpected end of file" na visualização do programa de indicações (`index.blade.php`), causado por uma diretiva `@if` não fechada. A correção restaura o carregamento completo do painel de afiliados.
- **Sincronização de Credenciais Mercado Pago (22/03):** Corrigido o erro "Erro ao carregar o gateway de pagamento" no checkout. A lógica de resolução da `publicKey` no `CheckoutController` foi sincronizada com o `MercadoPagoService` para garantir que, quando o vendedor for o administrador da plataforma, sejam utilizadas as credenciais globais corretas.
