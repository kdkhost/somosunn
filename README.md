...
- **Edição Rica para Páginas Jurídicas (24/03):** Habilitada a edição completa via Summernote para as páginas de **Termos de Uso, Política de Privacidade e Consentimento LGPD**. O administrador agora possui total controle sobre o conteúdo textual e formatação destas páginas em ambos os painéis (AdminLTE e Moderno), integrando-as ao motor de SEO Robusto e padronização de abas do sistema.
- **Restauração da Integridade Visual e Padronização de Abas (24/03):** Refatoração completa dos editores de página em ambos os painéis. O AdminLTE retornou ao padrão de **coluna única com abas nativas**, integrando o SEO Robusto como a aba principal. O Painel Moderno (Tailwind) foi padronizado com as configurações do sistema, utilizando **abas horizontais no topo** em vez de barra lateral, garantindo coerência visual e navegação "Zero Refresh" fluida em toda a plataforma.
- **Padronização de SEO e Metadados Robustos (24/03):** Implementação de um motor de SEO unificado em ambos os painéis (AdminLTE e Moderno). Agora, todas as páginas institucionais possuem suporte nativo a metadados avançados: Meta Keywords, Robots (Index/Follow), URL Canônica, Tipo de OpenGraph e imagens dedicadas para Social Sharing (OG Image e Twitter Card). A interface foi redesenhada com contadores de caracteres em tempo real e visualização premium de cards sociais, garantindo que a plataforma atinja o nível máximo de otimização para motores de busca e compartilhamento em redes sociais.
- **Sincronização de Controladores e Campos Escalares (24/03):** Refatoração profunda na arquitetura de controllers (`PageController`) para utilizar constantes de campos compartilhados (`SHARED_SCALAR_FIELDS`), garantindo que qualquer nova melhoria de SEO seja replicada automaticamente em todas as páginas do sistema sem redundância de código.

- **Paridade Total e Modernização de Painéis (24/03):** Alcançada a paridade funcional absoluta entre o painel moderno (Tailwind) e o legado (AdminLTE 3.2). Todos os partials institucionais (`Home`, `Quem Somos`, `Valores`, `Como Funciona`) no painel legado foram migrados para o novo **Motor de Repeater Visual**. Administradores agora desfrutam de uma experiência unificada com Drag-and-Drop, Upload de fotos via AJAX e edição em tempo real, eliminando definitivamente a necessidade de lidar com JSON bruto em qualquer interface.
- **Navegação "Zero Refresh" (SPA-like) (24/03):** Implementação de um sistema de navegação por abas inteligente em ambos os painéis administrativos. Ao navegar entre as seções da página de edição, o sistema alterna a visibilidade instantaneamente via JavaScript sem recarregar a página, mantendo o estado da URL e garantindo uma fluidez de uso premium e sem interrupções.
- **Modernização Avançada do CMS (24/03):** Implementada a funcionalidade de **Drag-and-Drop (D&D)** via SortableJS em todos os Repeaters administrativos, permitindo a reordenação visual de depoimentos, equipe e passos de forma intuitiva. Foi integrado também o **Upload de Imagens AJAX** diretamente nos cards do Repeater com preview em tempo real, agilizando a gestão de fotos de fundadores e membros da equipe sem sair da interface de edição.
- **Preview de E-mail Premium e Resiliente (24/03):** Refatoração da lógica de pré-visualização para garantir compatibilidade total com o tema escuro (dark mode) do painel moderno. A nova implementação utiliza `json_encode` para integridade de dados e `callbacks` nativos do Summernote, assegurando que o card do e-mail seja renderizado instantaneamente com alta fidelidade visual e sem conflitos de cores de fundo.
